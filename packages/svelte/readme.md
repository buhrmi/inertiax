...
| `visitOptions` | `VisitOptions` | `{ replace: true, updateBrowserUrl: false, preserveScroll: true }` (non-top) / `{ replace: false, updateBrowserUrl: true, preserveScroll: false }` (top) | Default visit options applied to all navigations within the frame. Link/form-level options take precedence. |
